...
4. Provide each worker: milestone description, full context, branch name, files in scope, verify command (`npm test`), and these instructions:
   - Implement only the files in your milestone scope
   - Run `npm test` before reporting done — fix any failures
...
6. When a worker completes: review the changes, run `npm test`, fix integration issues if needed
