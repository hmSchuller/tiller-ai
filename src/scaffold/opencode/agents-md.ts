...
export function generateAgentsMd(_config: ProjectConfig): string {
...
3. **Build** — implement milestone by milestone; milestones tagged \`[independent]\` may be parallelized using agent teams; each milestone includes tests, verify, and auto-commit
...
- **/setup** — first-run: understand the project and configure project instructions
...
Tiller provides four specialist agents. They are spawned by skills as subagents.

- **quartermaster** — independent code reviewer. Spawned at end of sail to review the feature branch diff. Returns PASS or FAIL. Negotiates one round; escalates to Captain on impasse.
...
- **captain** — arbitration. Only activated when Quartermaster and Sailing Agent reach impasse. Issues one of three rulings: AGREE WITH QUARTERMASTER, AGREE WITH SAILING AGENT, or COMPROMISE.
