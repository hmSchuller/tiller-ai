...
export function generateCopilotDockSkill(config: ProjectConfig): string {
  return `---
name: dock
...
**Show the post-dock sail prompt only after a successful dock. All Step 8 user interaction uses \`AskUserQuestion\` only — no chat prompts.**
...
## Step 8: Confirm and optionally start a new sail

\`TaskUpdate\` → mark "Confirm" as \`in_progress\`.

After the dock completes successfully:

- **simple:** Use \`AskUserQuestion\` to ask: "Done. What next?" with options ["Start new sail", "Finish"]
- **detailed:** Use \`AskUserQuestion\` to ask: "Feature docked. What next?" with options ["Start new sail", "Finish"]

If the user chooses **"Finish"**:
- End cleanly. Do not run \`/sail\`.

If the user chooses **"Start new sail"**:
1. Use \`AskUserQuestion\` to ask: "What should we work on next?" (freeform)
2. Treat the response as the new \`$ARGUMENTS\`
3. Do NOT auto-run \`/sail\`
4. Instead, read \`.github/skills/sail/SKILL.md\`, create a fresh progress checklist for the new sail, and continue into the Copilot sail flow from **Step 2** (branch routing) using the new task
